...
public class XMLFeatureModelParserSample {
	...
	public void parse() {
		...
		try {
			...
			String featureModelFile = "e:\\Users\\workspace\\Research\\src\\SPLOT-3CNF-FM-500-50-1.00-SAT-5.xml";
			...
			traverseDFS(featureModel.getRoot(), 0, pfm);
	...
	public void traverseDFS(FeatureTreeNode node, int tab, ParsedFM pfm) {
		for( int j = 0 ; j < tab ; j++ ) {
			System.out.print("\t");
		}
		System.out.print(index);
		node.attachData(index);
		// Root Feature
		...
		// Solitaire Feature
			...
			// Mandatory Feature
				...
				pair[0] = (Integer) parent.getAttachedData(); //the feature that requires
		...
		// Feature Group
			...
			pfm.addGroup(index, (FeatureGroup)node);
		}
		// Grouped feature
		else {
		...
		for( int i = 0 ; i < node.getChildCount() ; i++ ) {
			traverseDFS((FeatureTreeNode )node.getChildAt(i), tab+1, pfm);
		}
	}
