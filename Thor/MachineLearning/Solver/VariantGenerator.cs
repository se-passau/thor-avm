...
namespace MachineLearning.Solver
{
    public class VariantGenerator : IVariantGenerator
    {
       ...
       // CheckConfigSAT ccs = null;
        ...
        public List<List<BinaryOption>> GenerateRLinear(VariabilityModel vm, int treshold, int SolverTimeout, BackgroundWorker worker)
        {
            foreach (Lazy<IVariantGenerator, ISolverType> solver in solvers)
            {
                if (solver.Metadata.SolverType.Equals("MSSolverFoundation"))
                    return solver.Value.GenerateRLinear(vm, treshold, SolverTimeout, worker);
            }
