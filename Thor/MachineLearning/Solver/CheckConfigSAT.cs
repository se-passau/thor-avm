...
namespace MachineLearning.Solver
{
    public class CheckConfigSAT : ICheckConfigSAT
    {
        ...
        public bool checkConfigurationSAT(List<BinaryOption> config, VariabilityModel vm, bool exact)
        {
            ...
            {
                if (solver.Metadata.SolverType.Equals("MSSolverFoundation")) return solver.Value.checkConfigurationSAT(config, vm, exact);
            }

            //If not MS Solver, take any solver. Should be changed when supporting more than 2 solvers here
            foreach (Lazy<ICheckConfigSAT, ISolverType> solver in solvers)
            {
                return solver.Value.checkConfigurationSAT(config, vm, exact);
            }

            return false;
        }

        public bool checkDimacsSAT(List<string> tempConfigs, string[] lines)
        {
            Console.WriteLine("Test");
            foreach (Lazy<ICheckConfigSAT, ISolverType> solver in solvers)
            {
                if (solver.Metadata.SolverType.Equals("MSSolverFoundation")) return solver.Value.checkDimacsSAT(tempConfigs, lines);
            }

            foreach (Lazy<ICheckConfigSAT, ISolverType> solver in solvers)
            {
                return solver.Value.checkDimacsSAT(tempConfigs, lines);
            }
        ...
        private static CheckConfigSAT ccs = new CheckConfigSAT(AppDomain.CurrentDomain.BaseDirectory);
    }
}
