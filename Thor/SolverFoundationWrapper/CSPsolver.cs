...
namespace MicrosoftSolverFoundation
{
    class CSPsolver
    {
        ...
        internal static ConstraintSystem getDimacsCSystem(out List<CspTerm> variables,
            out Dictionary<string, CspTerm> optToTerm, out Dictionary<CspTerm, string> termToOpt, string[] lines)
        {
            var sys = ConstraintSystem.CreateSolver();
            variables = new List<CspTerm>();
            optToTerm = new Dictionary<string, CspTerm>();
            termToOpt = new Dictionary<CspTerm, string>();
            ...
            foreach (var line in lines)
            {
                ...
                {
                    var domain = sys.DefaultBoolean;
                    var name = line.Split(null)[2];
                    var temp = sys.CreateVariable(domain, name);

                    optToTerm.Add(name, temp);
                    termToOpt.Add(temp, name);
                ...
                else
                {
                    ...
                    var literals = new CspTerm[splits.Length - 1];

                    /*if (literals.Length == 1)
                    {
                        var val = Convert.ToInt32(splits[0]);
                        if (val < 0)
                        {                        
                            var lit = sys.Not(variables[Math.Abs(val) - 1]);
                            sys.AddConstraints(lit);
                        }
                        else
                        {
                           
                            var lit = variables[val - 1];
                            sys.AddConstraints(lit);
                        }
                    } 
                    else*/
                    {
                        ...
                        {
                            var val = Convert.ToInt32(splits[i]);
                            if (val < 0)
                                literals[i] = sys.Not(variables[Math.Abs(val) - 1]);
                            else
                            {
                                literals[i] = variables[val - 1];
                            }
                        }
                        sys.AddConstraints(sys.Or(literals));
