...
public class FM72r extends Problem {   

  private static ParsedFM featureModel;
    
  public FM72r(String solutionType) throws ClassNotFoundException {
    this(solutionType, 72, 8, new ParsedFM("..\\trunk2\\jmetal\\problems\\FMr\\FM-72.xml"));
  }

  public FM72r(String solutionType, 
   ...
   // } //for
    ...
    //public static final ParsedFM featureModel = new ParsedFM("src\\jmetal\\problems\\FMr\\FM-72.xml");
    public static final int FEATURES = 72;
    public static final int [] [] requires_pairs = {{63,24} , {71,38} , {47,25} , {11,3} , {10,6}
                                                    , {70,69} , {17,20} , {18,21}};
    
    public static ObjectiveParser op = new ObjectiveParser("..\\trunk2\\jmetal\\problems\\FMr\\qualities72.csv");
  ...
  */    
  public void evaluate(Solution solution) throws JMException {
        ...
        // 10 rules in this FM
        if (variable.bits_.get(18) && variable.bits_.get(20)) excludes_viol++;
        if (variable.bits_.get(23) && variable.bits_.get(58)) excludes_viol++;
        if (variable.bits_.get(5) && variable.bits_.get(10)) excludes_viol++;
        if (variable.bits_.get(17) && variable.bits_.get(21)) excludes_viol++;
        if (variable.bits_.get(16) && variable.bits_.get(21)) excludes_viol++;
        if (variable.bits_.get(16) && variable.bits_.get(20)) excludes_viol++;
        if (variable.bits_.get(3) && variable.bits_.get(5)) excludes_viol++;
        if (variable.bits_.get(3) && variable.bits_.get(6)) excludes_viol++;
        if (variable.bits_.get(2) && variable.bits_.get(7)) excludes_viol++;
        if (variable.bits_.get(2) && variable.bits_.get(8)) excludes_viol++;
