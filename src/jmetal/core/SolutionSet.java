...
public long anyC = -1;
public void setAnyC(long anyC) {
	this.anyC = anyC;
}

public void setFiftyC(long fiftyC) {
	this.fiftyC = fiftyC;
}

public void setHundredC(long hundredC) {
	this.hundredC = hundredC;
}

public long fiftyC = -1;
public long hundredC = -1; 
  ...
  */
  ...
  public void printObjectivesToFile(String path){
    try {
      ...
      for (int i = 0; i < solutionsList_.size(); i++) {
    	...
    	if(solutionsList_.get(i).getObjective(0)==0.0) 
    		numCorrect++;				
        bw.write(solutionsList_.get(i).toString());
        bw.newLine();
        //}
      }
      //System.out.println(numCorrect + "/" + solutionsList_.size());
  ...
  public float getPercentCorrect() {
      ...
      float percent = (100f * ((float) numCorrect / (float) solutionsList_.size()));
      //System.out.println("Percent Correct: " + percent);
      return percent;
  }
  
  public void printCurrentObjectivesToFile(BufferedWriter bw, long time, double hv, double spread){
	    try {
	      double objSum[];
	      objSum = new double[12];
	      for(int j=0; j<12; j++){
	          objSum[j] = 0;
	      }
	      
	      double objAvg[];
	      objAvg = new double[12];

	      double objVar[];
	      objVar = new double[12];
	      for(int j=0; j<12; j++){
	          objVar[j] = 0;
	      }
	      ...
	      for (int i = 0; i < solutionsList_.size(); i++) {
	    	if(solutionsList_.get(i).getObjective(0)==0.0) 
	    		numCorrect++;	
	        for(int j=0; j<solutionsList_.get(0).numberOfObjectives(); j++){
	            objSum[j] += solutionsList_.get(i).getObjective(j);
	        }
	      }
	      float percent = (100f * ((float) numCorrect / (float) solutionsList_.size()));
	      
	      bw.write(time + "," + hv + "," + spread + "," + numCorrect + "," +
                      solutionsList_.size() + "," + percent + ","); //time hv spread percent
	      System.out.print(time + "," + hv + "," + spread + "," + numCorrect + "," +
                      solutionsList_.size() + "," + percent + ",");
              
              for(int j=0; j<solutionsList_.get(0).numberOfObjectives(); j++){
                objAvg[j] = objSum[j]/solutionsList_.size();
              }
              
	      for (int i = 0; i < solutionsList_.size(); i++) {
	        for(int j=0; j<solutionsList_.get(0).numberOfObjectives(); j++){
	            double temp = solutionsList_.get(i).getObjective(j) - objAvg[j];
	            objVar[j] += temp * temp;
	        }
	      }
	      ...
	      for(int j=0; j<solutionsList_.get(0).numberOfObjectives(); j++){
	    	  //AVG STDEV
	          bw.write(objAvg[j] + "," + Math.sqrt(objVar[j]/solutionsList_.size()) + ",");
                  System.out.print(objAvg[j] + "," + Math.sqrt(objVar[j]/solutionsList_.size()) + ",");
	      }
	      bw.newLine();
              System.out.print("\n");
	      bw.flush();
	  ...
	  } // printCurrentObjectivesToFile
  
  public String toString() {
	  String result = "";
	  for (int i=0;i<solutionsList_.size(); i++) {
		  result+=solutionsList_.get(i) + "\n";
	  }
	  return result;
  }
  ...
  public void printVariablesToFile2(String path, ParsedFM featureModel){
	    try {
	      /* Open the file */
	      FileOutputStream fos   = new FileOutputStream(path)     ;
	      OutputStreamWriter osw = new OutputStreamWriter(fos)    ;
	      ...
	      for (int i = 0; i < solutionsList_.size(); i++) {  
	      	for (int j = 0; j < numberOfVariables; j++) {
	      	  //System.out.println(featureModel.correctString((Binary) solutionsList_.get(i).getDecisionVariables()[j]));
	          bw.write(solutionsList_.get(i).getDecisionVariables()[j].toString() + " ");
	      	}
	        bw.newLine();        
	      }
...
public void setTimeCorrect(long anyC2, long fiftyC2, long hundredC2) {
	anyC = anyC2;
	fiftyC = fiftyC2;
	hundredC = hundredC2;
}
