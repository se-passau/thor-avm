...
public class Binary extends Variable {
  ...
  public Binary(String initialString) {
	  //System.out.println(initialString);
	  bits_ = new BitSet(initialString.length());
	  numberOfBits_ = initialString.length();
	  id = -999;
	  for(int i=0;i<initialString.length();i++) {
		  int b = (int) Character.getNumericValue(initialString.charAt(i));
		  if(b==0) {
			  bits_.set(i,false);
		  }  
		  else
