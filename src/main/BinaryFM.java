 ...
 * or real variables).
 */ 
public class BinaryFM extends Variable {
   ...
   */
  public BinaryFM(int numberOfBits){   
  ...
  public BinaryFM(String initialString) {
	  bits_ = new BitSet(numberOfBits_);
	  for(int i=0;i<initialString.length();i++) {
		  int b = Character.getNumericValue(initialString.charAt(i));
		  if(b==0)
			  bits_.set(i,false);
		  else
			  bits_.set(i,true);
   ...
   */
  public BinaryFM(BinaryFM variable){
  ...
  */
  public int hammingDistance(BinaryFM other) {
    int distance = 0;
    int i = 0;
    while (i < bits_.size()) {
